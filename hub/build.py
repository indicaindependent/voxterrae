...
<tr><td>GitHub Releases</td><td><span class="pill">live</span></td><td><a href="https://github.com/indicaindependent/voxterrae/releases" rel="noopener">Same files, same hashes</a>, built there by the release workflow. Provenance attestation: 0.1.0 yes, 0.1.1 no (tagged while the repository was briefly private).</td></tr>
...
roadmap = """<h2>Roadmap</h2><ul><li><strong>0.1.0</strong> (shipped): Windows portable exe and source, SHA256SUMS, GitHub release with provenance attestation, in-app update check against <span class="mono">/api/latest.json</span>. The exe crashed at launch and was withdrawn.</li><li><strong>0.1.1</strong> (shipped): the exe starts; the release workflow now runs the built exe before publishing. No attestation for this one build.</li><li><strong>0.2</strong>: SignPath Foundation code signing application; winget manifest (portable).</li><li><strong>0.3</strong>: <span class="mono">voxterrae://join/</span> deep links from warheatmap.app event cards; live-map rail fed by the event stream.</li><li><strong>Later</strong>: MSI installer, macOS/Linux packages.</li></ul>"""
