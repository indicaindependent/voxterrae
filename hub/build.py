...
.fine{{color:var(--muted);font-size:14px}}.iimbadge{{display:block;margin-top:14px;max-width:420px}}.iimbadge img{{width:100%;height:auto;border-radius:12px;display:block;box-shadow:0 10px 26px -14px rgba(0,0,0,.6)}}
.shot{{border:1px solid var(--line);border-radius:12px;overflow:hidden;background:var(--panel);box-shadow:0 30px 80px rgba(0,0,0,.5)}}.shot img{{display:block;width:100%;height:auto}}
...
def asset_rows(rel):
    ...
    for a in rel.get("withdrawn", []):
        rows.append(f"<tr><td>{a['file']} <span class=\"pill\">withdrawn</span><div class=\"fine\">{html.escape(a.get('reason',''))}</div></td><td>{a['platform']}</td><td>{a['size']/1048576:.1f} MB</td><td><span class=\"mono\">{a['sha256']}</span> <span class=\"fine\">(not served)</span></td></tr>")
...
# ---- home ------------------------------------------------------------------------------------
src = next(a for a in LATEST["assets"] if a["kind"]=="source")
exe = next((a for a in LATEST["assets"] if a["kind"]=="portable"), None)
...
<p class="fine">{("Windows portable exe (" + f"{exe['size']/1048576:.0f}" + " MB, built by the public GitHub Actions workflow" + (" with a provenance attestation" if exe.get('attested') else "; this build has no provenance attestation, see the release notes") + ") and the source bundle (" + f"{src['size']/1024:.0f}" + " KB), both with SHA-256 published.") if exe else ("The Windows exe for this version is not available. Today's asset is the source bundle (" + f"{src['size']/1024:.0f}" + " KB, SHA-256 published).")} <a href="/docs/install">Run from source</a> also works.</p></div>
<figure class="shot" style="margin:0"><img src="/img/window-012.png" width="1280" height="720" alt="VoxTerrae 0.1.2 window: room list grouped by HOME and EFnet with unread counts on the left, the #warheatmap timeline with coloured nicks, reactions and a reply strip in the middle, the live-map tile and member list with rank badges on the right, network status and lag in the bar at the bottom" loading="eager" fetchpriority="high"></figure></section>
...
<h2>Trust, in the open</h2><p>Every file carries a published SHA-256 and a verify command. Certificates on EFnet are pinned on first use and the client refuses a changed one. Source is MIT-licensed and public at <a href="https://github.com/indicaindependent/voxterrae" rel="noopener">github.com/indicaindependent/voxterrae</a>; every release is built there in the open, and a build provenance attestation is published whenever the build could issue one (each release page says whether it did). Code signing is on the <a href="/releases">roadmap</a>, and this page says so rather than hiding the SmartScreen prompt.</p>"""
...
Get-FileHash .\\{(exe or src)['file']} -Algorithm SHA256
# expected
{(exe or src)['sha256']}
{("# build provenance (GitHub CLI), for the exe: proves the bytes came from the public release workflow" + chr(10) + "gh attestation verify ." + chr(92) + "VoxTerrae.exe --repo indicaindependent/voxterrae") if (exe and exe.get('attested')) else ("# this release has no provenance attestation (see its release notes); the SHA-256 above is the check")}</code></pre><p>Details and what a checksum does and does not prove: <a href="/docs/verify">Verify a download</a>.</p>
<h2>Other channels</h2><table><thead><tr><th>Channel</th><th>State</th><th>Notes</th></tr></thead><tbody>
<tr><td>GitHub Releases</td><td><span class="pill">live</span></td><td><a href="https://github.com/indicaindependent/voxterrae/releases" rel="noopener">Same files, same hashes</a>, built there by the release workflow. Provenance attestation: 0.1.0 and 0.1.2 yes; 0.1.1 no (tagged while the repository was briefly private).</td></tr>
...
roadmap = """<h2>Roadmap</h2><ul><li><strong>0.1.0</strong> (shipped): Windows portable exe and source, SHA256SUMS, GitHub release with provenance attestation, in-app update check against <span class="mono">/api/latest.json</span>. The exe crashed at launch and was withdrawn.</li><li><strong>0.1.1</strong> (shipped): the exe starts; the release workflow now runs the built exe before publishing. No attestation for this one build.</li><li><strong>0.1.2</strong> (shipped): Windows 11 polish and hardening (title bar, tray, single instance, crash log), timeline and composer work, settings, quick switcher, measured lag; the autojoin rule now has a test on it; attestation is back.</li><li><strong>0.2</strong>: SignPath Foundation code signing application; winget manifest (portable).</li><li><strong>0.3</strong>: <span class="mono">voxterrae://join/</span> deep links from warheatmap.app event cards; live-map rail fed by the event stream.</li><li><strong>Later</strong>: MSI installer, macOS/Linux packages.</li></ul>"""
...
<h2>Who makes it</h2><p>VoxTerrae is made by <a href="https://osintnet.uk" rel="noopener">Indica Independent Media</a>: independent OSINT and open tooling, in the VPDLNY tradition. Information serves people, never the other way around. No trackers, no data sale, no dark patterns.</p>
<a class="iimbadge" href="https://osintnet.uk" rel="noopener" title="Created with Creative Clarity. A unique badge on every visit."><img loading="lazy" alt="Created with Creative Clarity: Indica Independent Media" width="400" height="200" src="https://badge.osintnet.uk/badge.svg?dynamic"></a>"""
...
const PAGES = %s;
const SEC = { "x-content-type-options": "nosniff", "referrer-policy": "strict-origin-when-cross-origin", "x-frame-options": "DENY", "permissions-policy": "camera=(), microphone=(), geolocation=()", "strict-transport-security": "max-age=31536000; includeSubDomains; preload", "content-security-policy": "default-src 'none'; img-src 'self' data: https://badge.osintnet.uk; style-src 'unsafe-inline'; script-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'" };
