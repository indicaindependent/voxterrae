...
## EFnet
A 1990s network with no services and no message ids. Reactions and replies are not possible there, so the client switches to **classic mode**: plain lines, `/me`, quoting instead of replying. Five community rooms are joined automatically: `#phpnuke #wk #OGhomecoming #ComebacktoIRC #VetsHateDiscord`.
