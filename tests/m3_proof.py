...
async def run():
    ...
    await asyncio.sleep(3); await br.sessions["home"].client.join("#vt-m0-test"); await asyncio.sleep(2)
    win.show_room("home/#vt-m0-test")
    other = IrcClient(ClientConfig(host="irc.warheatmap.app", nick="vtpeer" + secrets.token_hex(2))); await other.connect(); await other.join("#vt-m0-test"); await asyncio.sleep(1.5)
    # typing from us
    ...
    pmid = await other.privmsg("#vt-m0-test", f"hey {handle} look at this {token}"); await asyncio.sleep(2.5)
    notified = (win.unread_total, win.windowTitle(), win.rooms._unread.get("home/#vt-m0-test"))
    win.show_room("home/#vt-m0-test")
    # we reply + react through the UI signals
    items = [i for i in win.models["home/#vt-m0-test"].items if i.kind == "msg" and token in i.text]
    assert items and items[0].highlight, f"mention not highlighted: {[(i.nick, i.text) for i in win.models['home/#vt-m0-test'].items if i.kind=='msg'][-3:]}"
    win.set_reply(items[0]); win.composer.setText(f"got it {token}"); win._send(); await asyncio.sleep(2.5)
    win.send_react.emit("home/#vt-m0-test", items[0].msgid, "🔥"); await asyncio.sleep(2)
    msgs = [i for i in win.models["home/#vt-m0-test"].items if i.kind == "msg"]
    ...
    print("M3 PROOF:", {"notify(unread,title,room_badge)": notified, "after_switch_title": win.windowTitle(), "mention_msgid": pmid,
          ...
          "store_reactions": br.store.reactions("home", "#vt-m0-test", pmid), "read_marker": br.store.read_marker("home", "#vt-m0-test") is not None,
          "tray": bool(win.tray)})
