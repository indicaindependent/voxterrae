...
async def run():
    ...
    await br.sessions["home"].client.join("#vt-m0-test"); await asyncio.sleep(2); win.show_room("home/#vt-m0-test")
    other = IrcClient(ClientConfig(host="irc.warheatmap.app", nick="cartographer" + secrets.token_hex(1))); await other.connect(); await other.join("#vt-m0-test"); await asyncio.sleep(1.5)
    await other.privmsg("#vt-m0-test", "new event card up for the Hormuz lane, anyone watching the tanker count tonight?"); await asyncio.sleep(1)
    pmid = await other.privmsg("#vt-m0-test", f"{handle} the map shows 3 transits since 18:00, does the desk brief agree? {token}"); await asyncio.sleep(2)
    ...
    win.send_react.emit("home/#vt-m0-test", items[0].msgid, "👀"); await asyncio.sleep(1.5)
    await other.privmsg("#vt-m0-test", "that turnaround is the one the OSINT thread flagged, good catch"); await asyncio.sleep(1.5)
    last = [i for i in win.models["home/#vt-m0-test"].items if i.kind == "msg"][-1]
